...
| `src/runtime/components/ECodeBlock.ts`, `code-block-themes.ts`, `code-block-languages.ts` | `packages/react-email/src/components/code-block/{code-block.tsx,prism.ts,themes.ts}` | PrismJS tokenization, the no-break-space/zero-width-joiner/zero-width-space encoding, per-line `<br/>` output, and the 38 theme CSS-value maps are reproduced byte-for-byte. PrismJS itself is a runtime dependency; see `THIRD_PARTY_NOTICES`. |
