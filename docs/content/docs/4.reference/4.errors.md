...
| --- | --- | --- |
| `EmailRenderError` | Component loading, prop validation, Vue SSR, document validation, or plain-text conversion fails. | `templateName` (the registry name for `renderEmail()`, or the Vue component name for standalone rendering); original failure on `cause`. |
