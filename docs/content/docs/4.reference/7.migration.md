...
Nuxt Email stops at `{ html, text, subject? }`. Keep recipients, sender identity, provider credentials, attachments, tags, scheduling, and delivery policy in your provider SDK. There is no provider-neutral adapter or public send endpoint, so migrating does not require giving up provider-specific features. You can optionally compute the subject line beside the template with [`defineEmail`]($docs/guides/subject-lines).
