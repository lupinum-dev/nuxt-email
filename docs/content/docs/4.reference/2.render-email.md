...
```ts [server/utils/render-transactional-email.ts]
...
The conversion excludes head content, images, script and style content, and elements marked `data-skip-in-text="true"`. This removes `EPreview` from the fallback. Links retain destinations when their visible text differs. See the [plain-text guide]($docs/guides/plain-text) for details and the `data-text-format="dataTable"` opt-in.
