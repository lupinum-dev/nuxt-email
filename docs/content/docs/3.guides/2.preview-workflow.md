...
Render failures show the template name, the `EmailRenderError` stack, and its original cause. Use the [error reference]($docs/reference/errors) to find the recovery step by symptom.
...
The development endpoints cannot become production render or send APIs. Use the typed server-only [`renderEmail`]($docs/reference/render-email) in your handlers.
