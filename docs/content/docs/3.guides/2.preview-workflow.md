...
Render failures show the template name, the wrapped `EmailRenderError` stack, and its original cause. Use the [error reference]($docs/reference/errors) to find the recovery step by symptom. A template without a fixture stays visible in the list but cannot be rendered until its sibling fixture is added.
