...
Shiki remains an installation-time dependency of `@lupinum/nuxt-email`. The option controls build and runtime cost, not package download cost. Discovery directories, the preview route, and the render API remain fixed. See the [module behavior reference]($docs/reference/module) for the full contract.
