...
| Field | Value |
| --- | --- |
| Nuxt Email version | `0.1.0` release candidate |
...
2. Create one representative Vue SFC under `app/emails/` using ordinary `defineProps()`, slots, and supported `E*` components.
...
| API change required before v0.1 | Pending beta run |
...
An external beta is accepted only when no known data-loss, injection, production-bundle, invalid-document, or immediately breaking API defect remains. Feature requests outside the frozen v0.1 surface are roadmap input, not release blockers unless they prevent the representative transaction.
