export default {
  ginkoDocs: {
    ...
    landing: {
        ...
        media: {
          ...
          tabs: [
            ...
            },
            {
              label: { en: 'Render' },
              icon: 'lucide:server',
              filename: 'server/api/welcome.get.ts',
              language: 'ts',
              code: [
                'export default defineEventHandler(async () => {',
                '  // renderEmail is a Nitro-only auto-import, typed from app/emails/',
                '  const { html, text } = await renderEmail(\'welcome\', {',
                '    firstName: \'Ada\',',
                '    activationUrl: \'https://example.com/activate\',',
                '  })',
                '',
                '  // hand html + text to your provider SDK',
                '  return { html, text }',
                '})',
              ].join('\n'),
            },
      ...
      install: { command: 'pnpm add nuxt-email' },
      features: [
        {
      ...
      cta: {
        title: { en: 'Author your first email in a few minutes.' },
        secondary: {
