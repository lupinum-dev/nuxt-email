...
It is a **Nitro auto-import** — available in server handlers and other Nitro server code, not in Vue components, client plugins, or production client bundles. Do not import it from `nuxt-email` or `#imports`.
...
- There is no v0.1 raw-HTML component. Do not use `v-html` or native raw HTML with untrusted content inside a template.
