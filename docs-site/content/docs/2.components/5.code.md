...
| Prop | Contract |
| --- | --- |
| `code` | Required source string. Content is escaped before rendering. |
| `language` | Required configured Shiki language. It is typed from the allowlist and checked again at runtime. |
| `lineNumbers` | Optional boolean, default `false`. Line numbers are decorative and excluded from the plain-text alternative. |
| `fontFamily` | Optional monospace font stack applied inline to the block and tokens. |
