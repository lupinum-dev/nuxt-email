...
The theme objects (`dracula`, `oneDark`, `nord`, …) are named exports of `@lupinum/nuxt-email/themes` and are imported into your template's `<script setup>`. The `ECodeBlock` component itself is auto-registered and is not imported.
::
