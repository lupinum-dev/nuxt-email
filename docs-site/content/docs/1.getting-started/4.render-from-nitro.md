...
Do not import `renderEmail` from `nuxt-email` or `#imports`; it is injected as a server auto-import. TypeScript checks the template name `'welcome'` and its exact props.
