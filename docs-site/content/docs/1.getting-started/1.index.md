...
Nuxt `4.4.8` is the verified compatibility baseline. Nuxt `4.5.x` is explicitly excluded because its Vite builder imports an undeclared dependency before this module loads. Widen the peer range only after an isolated consumer passes. CI covers Node 22, 24, and 26. Other Node majors, Nuxt 3, Nuxt 5, edge runtimes, and client-side email rendering are outside the support contract.
