...
You do not import the `E*` components — the module auto-imports all eighteen into email templates. Use normal `defineProps()`, slots, `v-if`, `v-for`, Tailwind v4 classes, HTML attributes, and Vue style bindings. `defineEmail` is an explicit server-only import; its closure captures the template's actual props.
